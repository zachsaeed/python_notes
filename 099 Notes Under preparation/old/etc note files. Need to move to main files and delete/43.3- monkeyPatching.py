...
# Monkey patching is a technique that allows you to alter the behavior of objects at runtime. Even though it can be a
# very useful feature, it can also make your code much harder to understand and debug, and therefore you have to be
# careful with how you implement monkey patching. Here, we are going to see some examples of how you can use monkey
# patching to solve quickly specific problems. We are also going to discuss the consequences of monkey patching in the
# context of larger projects.

# Monkey patching is tightly related to the idea of mutability in Python. Custom objects are mutable, and therefore
# their attributes can be replaced without creating a new copy of the object.
...
# If we go line by line, you see that we create an object using MyClass and we call it var1. We then copy the object to
# another variable, called var2. We change the values stored in var1, but we observe that the values stored in var2 have
# also changed. This is simply because, in Python, the object variable is only a reference to the object. In the line
# var2 = var1 we have just copied the reference, but both are pointing to the same underlying object.

# Python also allows you to change attributes in the class itself, not in the instance of the class.
...
# What we see is that if we directly alter the value of any of the attributes of the class, the instances inherit this
# change. This is both very useful and very dangerous, since you may be altering the value of attributes of objects
# which you were not intending to modify. There is one last behavior that is important to point out, and refers to
# mixing the two approaches we have followed before:
var1 = MyClass()
var2 = var1
...
print(var1.a)  # 2
print(var2.a)  # 2
# Even if you change the attribute a to 3, you don't see this change appearing on the instances of the class. The root
# cause of this lays in the ideas behind mutable and immutable data types in Python. Since you altered the value of
# var1.a, now the attribute is pointing to an object different from the object the class attribute points to. If this
# last line doesn't make sense, go to the articles linked earlier on mutable and immutable data types.
'''
Finally, the last case I wanted to point out is what happens if you keep a reference to the attribute a before you 
modify it:
